...
let login_token = '';
let phone = process.env.xm_phone, password = process.env.xm_pwd
...
async function login1(phone, password) {
  const options = {
    "url": `https://api-user.huami.com/registrations/+86${phone}/tokens`,
    "body": `client_id=HuaMi&password=${password}&redirect_uri=https://s3-us-west-2.amazonaws.com/hm-registration/successsignin.html&token=access`,
    "headers": {
    ...
    request.post(options, async (err, resp, data) => {
      try {
        if (err) {
          ...
          let headers = resp.toJSON().headers;
          let location = headers.location
          let reg = new RegExp("(?<=access=).*?(?=&)");
          // reg.exec(location)
          let code = reg.exec(location)[0]
          await login2(code)
        }
      } catch (e) {
        $.logErr(e, resp)
    ...
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data) {
            console.log('重新登录成功...')
            let loginToken = JSON.parse(data).token_info.login_token
            // let user_id = JSON.parse(data).token_info.user_id
            $.tokenInfo = JSON.parse(data);
            //存储数据
            $.setdata(loginToken, 'xmSportsToken');
            if ($.tokenInfo.result === 'ok') {
              const { app_token, user_id } = $.tokenInfo.token_info;
              await get_time();
              await change_step(app_token, user_id);
              if ($.changeStepRes && $.changeStepRes.code === 1) {
