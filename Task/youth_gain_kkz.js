...
function checkStatus(d, e) { try { var b = d, g = new Date().getDay(); var c = b.getdata('ReadStatus') || '{}'; var f = b.getdata('CYCLE') || `{"youth_kkz":false,"youth_read":true,"jc_kkz":false,"jc_read":true}`; var a = { "isfinished": false, "day": 0, "running": false, "index": 0 }; const INDEX = { "youth_kkz": b.getdata('youth_start_index'), "youth_read": b.getdata('zqbody_index'), "jc_kkz": b.getdata('jckkz_index'), "jc_read": b.getdata('jcbody_index') }; var h = INDEX[e]; c = JSON.parse(c); f = JSON.parse(f); var i = c[e]; if (!isNull(i)) { a = i; if (a.day == g && a.isfinished && !f[e]) { b.msg("今天已经看完啦🎇~"); return false } var j = a.index; if (a.running && parseInt(h) > parseInt(j)) { b.msg("脚本正在运行中，本次退出🎇~"); a.running = true; a.isfinished = false; a.day = g; a.index = h; c[e] = a; b.setdata(JSON.stringify(c), 'ReadStatus'); return false } } a.running = true; a.isfinished = false; a.day = g; a.index = h; c[e] = a; b.setdata(JSON.stringify(c), 'ReadStatus'); return true } catch (error) { console.log('checkStatus===>', error); return false } } function setStatus(d, e) { var b = d, g = new Date().getDay(); var c = {}, f = {}; c.running = false; c.day = g; c.isfinished = true; c.index = 0; var a = b.getdata('ReadStatus'); f = JSON.parse(a); f[e] = c; b.setdata(JSON.stringify(f), 'ReadStatus') } function isNull(d) { if (!d) { return true } if (typeof d == 'object') { if (JSON.stringify(d) == '{}' || d.length == 0) { return true } } if (typeof d == 'string') { if (d == '{}') { return true } } return false };function checkStatus(d, e) { try { var b = d, g = new Date().getDay(); var c = b.getdata('ReadStatus') || '{}'; var f = b.getdata('CYCLE') || `{'youth_kkz':false,'youth_read':true,'jc_kkz':false,'jc_read':true,}`; var a = { "isfinished": false, "day": 0, "running": false, "index": 0 }; const INDEX = { 'youth_kkz': b.getdata('youth_start_index'), 'youth_read': b.getdata('zqbody_index'), 'jc_kkz': b.getdata('jckkz_index'), 'jc_read': b.getdata('jcbody_index') }; var h = INDEX[e]; c = JSON.parse(c); f = JSON.parse(f); var i = c[e]; if (!isNull(i)) { a = i; if (a.day == g && a.isfinished && !f[e]) { b.msg("今天已经看完啦🎇~"); return false } var j = a.index; if (a.running && parseInt(h) > parseInt(j)) { b.msg("脚本正在运行中，本次退出🎇~"); a.running = true; a.isfinished = false; a.day = g; a.index = h; c[e] = a; b.setdata(JSON.stringify(c), 'ReadStatus'); return false } } a.running = true; a.isfinished = false; a.day = g; a.index = h; c[e] = a; b.setdata(JSON.stringify(c), 'ReadStatus'); return true } catch (error) { console.log('checkStatus===>', error); return false } } function setStatus(d, e) { var b = d, g = new Date().getDay(); var c = {}, f = {}; c.running = false; c.day = g; c.isfinished = true; c.index = 0; var a = b.getdata('ReadStatus'); f = JSON.parse(a); f[e] = c; b.setdata(JSON.stringify(f), 'ReadStatus') } function isNull(d) { if (!d) { return true } if (typeof d == 'object') { if (JSON.stringify(d) == '{}' || d.length == 0) { return true } } if (typeof d == 'string') { if (d == '{}') { return true } } return false };