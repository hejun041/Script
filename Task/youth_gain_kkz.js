...
console.log(`\n === 脚本执行 ${bjTime} ===\n`);
!(async () => {
    if (!checkStatus($, 'youth_kkz', zq_threads - 1)) {
        return
    }
    $.log(`\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n您共提供${lookArr.length}次看看赚任务\n`)
    if (lookArr.length !== 0) {
        var promises = [];
        indexLasts = JSON.parse(indexLasts);
        if (indexLasts.length < zq_threads) {//下标初始化
            for (let i = indexLasts.length - 1; i < zq_threads; i++) {
                indexLasts[i] = 0;
            }
        } else if (indexLasts.length > zq_threads) {//下标初始化
...
async function doAction(lookArr, count, resolve) {
    var lookscore = 0;
    var begin = indexLasts[count - 1];//上次运行下标
    ...
    for (let k = begin ? parseInt(begin) : 0; k < lookArr.length; k++) {
        ...
        let currentIndexs = JSON.parse($.getdata('youth_kkz_indexs') || []);
        if (currentIndexs.length > indexLasts.length) {
            currentIndexs = currentIndexs.slice(0, indexLasts.length);
        }
        // else if (currentIndexs.length < indexLasts.length) {
        // }
        indexLasts = currentIndexs;
        indexLasts[count - 1] = index;
...
function checkStatus(e, c, g) { try { var b = e, d = new Date().getDay(), h = new Date().getHours(); if (h < 6) { b.msg(b.name, "太早啦~"); return false } if (h > 22) { b.msg(b.name, "太晚啦~"); return false } var f = b.getdata('ReadStatus') || '{}'; var j = b.getdata('CYCLE') || `{"youth_kkz":false,"youth_read":true,"jc_kkz":false,"jc_read":true}`; var a = { "isfinished": false, "day": 0, "running": false, "index": 0, "timeStamp": 0, "times": 0 }; const INDEX = { "youth_kkz": JSON.parse(b.getdata('youth_kkz_indexs')), "youth_read": b.getdata('zqbody_index'), "jc_kkz": b.getdata('jckkz_index'), "jc_read": b.getdata('jcbody_index') }; var i = INDEX[c], k; k = INDEX[c]; if (c == 'youth_kkz') { i = i[g] } f = JSON.parse(f); j = JSON.parse(j); var m = f[c]; if (!isNull(m)) { a = m; if (a.day != d) { a.times = 0 } if (a.day == d && a.isfinished && !j[c]) { b.msg(b.name, "今天已经看完啦🎇~"); return false } var l = a.index || 0; if (c == 'youth_kkz') { l = JSON.parse(l)[g] } var o = a.timeStamp; var n = new Date().getTime(); if (a.running && parseInt(i) > parseInt(l) && parseInt(n) - parseInt(o) < 2 * 60 * 1000) { b.msg(b.name, "脚本正在运行中，本次退出🎇~"); a.running = true; a.isfinished = false; a.day = d; if (c == 'youth_kkz') { k[g] = i; i = k } a.index = i; a.timeStamp = n; f[c] = a; b.setdata(JSON.stringify(f), 'ReadStatus'); return false } } a.running = true; a.isfinished = false; a.day = d; a.index = k; f[c] = a; b.setdata(JSON.stringify(f), 'ReadStatus'); return true } catch (error) { console.log('checkStatus===>', error); return false } } function setStatus(e, c) { var g = e; var b = new Date().getDay(); var d = {}; var h = { "running": false, "day": b, "isfinished": true, "index": 0, "times": 0 }; var f = g.getdata('ReadStatus'); d = JSON.parse(f); var j = d[c]; var a = j.times || 0; h.times = parseInt(a) + 1; d[c] = h; g.setdata(JSON.stringify(d), 'ReadStatus') } function setRunTime(e, c) { var g = e; var b = new Date().getTime(); var d = {}; var h = g.getdata('ReadStatus'); d = JSON.parse(h); var f = d[c]; f.timeStamp = b; d[c] = f; g.setdata(JSON.stringify(d), 'ReadStatus') } function isNull(e) { if (!e) { return true } if (typeof e == 'object') { if (JSON.stringify(e) == '{}' || e.length == 0) { return true } } if (typeof e == 'string') { if (e == '{}') { return true } } return false };