...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if ($.begin + 1 < lookStartbodyArr.length) {
        ...
        for (let k = indexLast ? indexLast : 0; k < lookStartbodyArr.length; k++) {
    ...
    return new Promise((resolve) => {
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                const result = JSON.parse(data)
                $.begin = $.begin + 1;
                let res = $.begin % lookStartbodyArr.length;
                $.setdata(res + "", 'jckkz_index');
...
function checkStatus(d, e) { try { var c = d, g = new Date().getDay(); var b = c.getdata('ReadStatus') || '{}'; var f = c.getdata('CYCLE') || `{"youth_kkz":false,"youth_read":true,"jc_kkz":false,"jc_read":true}`; var a = { "isfinished": false, "day": 0, "running": false, "index": 0, "timeStamp": 0 }; const INDEX = { "youth_kkz": c.getdata('youth_start_index'), "youth_read": c.getdata('zqbody_index'), "jc_kkz": c.getdata('jckkz_index'), "jc_read": c.getdata('jcbody_index') }; var h = INDEX[e]; b = JSON.parse(b); f = JSON.parse(f); var i = b[e]; if (!isNull(i)) { a = i; if (a.day == g && a.isfinished && !f[e]) { c.msg("今天已经看完啦🎇~"); return false } var j = a.index; var k = a.timeStamp; var l = new Date().getTime(); if (a.running && parseInt(h) > parseInt(j) && parseInt(l) - parseInt(k) < 2 * 60 * 1000) { c.msg("脚本正在运行中，本次退出🎇~"); a.running = true; a.isfinished = false; a.day = g; a.index = h; b[e] = a; c.setdata(JSON.stringify(b), 'ReadStatus'); return false } } a.running = true; a.isfinished = false; a.day = g; a.index = h; b[e] = a; c.setdata(JSON.stringify(b), 'ReadStatus'); return true } catch (error) { console.log('checkStatus===>', error); return false } } function setStatus(d, e) { var c = d; var g = new Date().getDay(); var b = {}; var f = { "running": false, "day": g, "isfinished": true, "index": 0, "times": 0 }; var a = c.getdata('ReadStatus'); b = JSON.parse(a); var h = b[e]; var i = h.times || 0; f.times = parseInt(i) + 1; b[e] = f; c.setdata(JSON.stringify(b), 'ReadStatus') } function setRunTime(d, e) { var c = d; var g = new Date().getTime(); var b = {}; var f = c.getdata('ReadStatus'); b = JSON.parse(f); var a = b[e]; a.timeStamp = g; b[e] = statusObj; c.setdata(JSON.stringify(b), 'ReadStatus') } function isNull(d) { if (!d) { return true } if (typeof d == 'object') { if (JSON.stringify(d) == '{}' || d.length == 0) { return true } } if (typeof d == 'string') { if (d == '{}') { return true } } return false };